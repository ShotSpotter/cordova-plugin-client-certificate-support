...
//                        new ProvSunTLSKDF().configure(BouncyCastleFipsProvider.this);
        ...
        private final static Object[] findSource()
        {
            ...
            for (int t = 0; t < initialEntropySourceNames.length; t++)
            {
                ...
                try
                {
                    ...
                    printAndroid("Found SecureRandom initial seed source: " + r[0].getClass().getName());
                    return r;
                }
                catch (ClassNotFoundException cnfe)
                {
                    printAndroid("Tested SecureRandom initial seed source: " + pair[0]);
                    continue;
                }
                catch (Throwable ex)
                {
            ...
            throw new RuntimeException("Could not instantiate platform SecureRandom.");
        }
    ...
    private static void printAndroid(String string)
    {
        try
        {
            Class cl = Class.forName("android.util.Log");
            cl.getMethod("w", new Class[]{String.class, String.class}).invoke(null, new Object[]{"BCFIPS", string});
        }
        catch (Throwable t)
        {
